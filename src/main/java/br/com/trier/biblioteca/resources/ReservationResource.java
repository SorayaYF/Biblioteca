...
public class ReservationResource {
	...
	public ResponseEntity<ReservationDTO> insert(@RequestBody ReservationDTO reservationDTO) {
		Reservation reservation = new Reservation(reservationDTO, bookService.findById(reservationDTO.getBookId()),
	...
	public ResponseEntity<List<ReservationDTO>> findByDateBetween(@PathVariable String dateIn,
			@PathVariable String dateFin) {
		return ResponseEntity.ok(
				service.findByDateBetween(dateIn, dateFin).stream().map(reservation -> reservation.toDTO()).toList());
	}
